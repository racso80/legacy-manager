...
import { compareSaveDates, isSupabaseConfigured, listCloudSaves } from "../cloud/cloudSaveService.js";
...
function Freshness({ local, cloud }) {
  const winner = compareSaveDates(local, cloud);
  const text = winner === "local" ? "Local más reciente" : winner === "cloud" ? "Nube más reciente" : "Sincronizadas";
  const color = winner === "local" ? "#f59e0b" : winner === "cloud" ? "#60a5fa" : "#22c55e";
      ...
      {saves.map(save => <div key={save.id} style={{ background:"#161a24", border:"1px solid rgba(255,255,255,.07)", borderRadius:11, padding:12 }}>
        <div style={{ display:"flex", justifyContent:"space-between", gap:10 }}>
          <div style={{ flex:1, minWidth:0 }}>
            ...
            {localSave?.cloudSaveId === save.id && <Freshness local={localSave.updatedAt} cloud={save.updated_at} />}
          </div>
        </div>
        <div style={{ display:"flex", gap:7, marginTop:10 }}>
