  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:12}}>
      ...
      {shown.map(player=>{const offer=activeByPlayer.get(player.id);const status=offer?statusMap[offer.status]??["•",offer.status,"#6b7280"]:null;return <div key={player.id} style={{background:"#161a24",border:`1px solid ${status?.[2]??"rgba(255,255,255,.06)"}33`,borderRadius:11,padding:11,marginBottom:8}}>
        ...
        {offer&&<div style={{marginTop:9,background:"#0d0f14",borderRadius:8,padding:9}}><div style={{fontSize:10,color:status[2],fontWeight:900}}>{status[0]} {status[1]}</div><div style={{fontSize:9,color:"#8b92a3",marginTop:4}}>Oferta: {fmt(offer.salary)}/sem · {offer.years} años · {offer.role}</div>{["salaryCounter","yearsCounter","roleCounter"].includes(offer.status)&&<button onClick={()=>onAcceptCounter(offer.id)} className="btn-gold" style={{width:"100%",marginTop:8,padding:8,borderRadius:8,fontSize:10}}>Aceptar petición del jugador</button>}{offer.status==="accepted"&&<button onClick={()=>{onComplete(offer.id);showFeedback(`Renovación firmada con ${player.name}.`,"success");}} className="btn-gold" style={{width:"100%",marginTop:8,padding:8,borderRadius:8,fontSize:10}}>Firmar renovación</button>}{!["completed","withdrawn"].includes(offer.status)&&<button onClick={()=>onWithdraw(offer.id)} className="btn-ghost" style={{width:"100%",marginTop:7,padding:7,borderRadius:8,fontSize:10}}>Retirar oferta</button>}</div>}
