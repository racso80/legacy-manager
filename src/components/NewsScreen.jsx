  ...
  return (
    <article onClick={()=>item.playerIds?.[0]&&onOpenPlayer?.(item.playerIds[0])} style={{
      ...
      <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:7 }}>
        ...
        <span style={{ marginLeft:"auto", fontSize:9, color:COLORS.textDim }}>Temp. {item.seasonLabel}{item.matchday ? ` · J${item.matchday}` : ""}</span>
      </div>
      ...
      {item.playerIds?.length>0 && <div style={{ display:"inline-block", marginTop:8, marginLeft:6, color:COLORS.muted, fontSize:9, fontWeight:700 }}>VER JUGADOR →</div>}
    </article>
  );
}
    ...
    <div style={{ flex:1, overflowY:"auto", padding:14 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, marginBottom:12 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:COLORS.textDim, marginTop:2 }}>{ { club:"La actualidad que afecta a tu equipo", league:"Resultados y clasificación en la competición", market:"Fichajes y movimientos de mercado", youth:"La cantera y el futuro del club", board:"Directiva, objetivos y economía" }[filter] ?? "Todo lo que se mueve en la competición" }</div>
        </div>
      ...
      {featured ? (
        ...
        <div style={{ textAlign:"center", padding:"55px 20px", color:COLORS.textDim }}>
          ...
          <div style={{ fontSize:14, color:COLORS.muted, fontWeight:700 }}>Todavía no hay noticias en esta sección</div>
