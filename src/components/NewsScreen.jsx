...
function NewsItem({ item, featured = false }) {
  ...
  return (
    <article style={{
      ...
      borderRadius: 10, padding: featured ? 16 : 12,
    }}>
...
export default function NewsScreen({ news = [], currentSeason }) {
    ...
    <div style={{ flex:1, overflowY:"auto", padding:14 }}>
      ...
      {featured ? (
        <>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>{rest.map(item => <NewsItem key={item.id} item={item} />)}</div>
        </>
      ) : (
