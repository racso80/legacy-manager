...
export default function TrainingCenterScreen({ game, onPlanChange, onOpenPlayer }) {
  ...
  return <div style={{flex:1,overflowY:"auto",padding:14}}>
    ...
    <div style={{fontSize:10,color:"#6b7280",fontWeight:800,letterSpacing:".6px",marginBottom:8}}>INFORME DE PROGRESIÓN</div>
    {report?<div style={{display:"flex",flexDirection:"column",gap:7}}>{[...game.players].sort((a,b)=>{const ac=changesByPlayer[a.id]?.changes?.length??0;const bc=changesByPlayer[b.id]?.changes?.length??0;return bc-ac||(b.potential-b.overall)-(a.potential-a.overall);}).slice(0,8).map(player=>{const item=changesByPlayer[player.id];const actual=item?.changes??[];const progress=item?.progress?.[0];return <button key={player.id} onClick={()=>onOpenPlayer(player,game.teamId)} style={{display:"flex",alignItems:"center",gap:9,textAlign:"left",background:"#161a24",border:"1px solid rgba(255,255,255,.06)",borderRadius:8,padding:10,cursor:"pointer"}}><div style={{width:34,height:34,borderRadius:7,background:"rgba(201,168,76,.1)",color:"#c9a84c",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800}}>{player.overall}</div><div style={{flex:1}}><div style={{color:"#e8eaf0",fontSize:11,fontWeight:700}}>{player.name} {player.age<=23&&player.potential-player.overall>=3?"🌱":""}</div><div style={{color:actual.length?"#22c55e":"#6b7280",fontSize:9,marginTop:3}}>{actual.length?actual.map(change=>`${change.label} ${change.delta>0?"+":""}${change.delta}`).join(" · "):progress?`${progress.label}: ${progress.value}% hacia la mejora`:"Sin progreso significativo"}</div></div><div style={{color:"#6b7280",fontSize:9}}>POT {player.potential}</div></button>})}<div style={{color:"#4b5563",fontSize:9,lineHeight:1.5,marginTop:5}}>El plan se aplica automáticamente después de cada jornada. Una carga mayor acelera el progreso, pero incrementa cansancio y riesgo médico.</div></div>:<div style={{background:"#161a24",borderRadius:9,padding:20,textAlign:"center",color:"#6b7280",fontSize:11}}>El primer informe llegará después de jugar una jornada.</div>}
  </div>;
}
