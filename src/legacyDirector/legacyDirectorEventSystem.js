...
function weeklyPreparationMoment(game, fixture, context = {}, activeEvents = []) {
  ...
  const candidates = [
    ...
    },
    {
      ...
      summary:`La carga de la semana es ${{ low:"baja", medium:"media", high:"alta", veryHigh:"muy alta" }[trainingLoad] ?? trainingLoad}. Propone enfocar la semana en ${recommendedFocus === "recovery" ? "recuperacion" : recommendedFocus === "highPress" ? "presion alta" : recommendedFocus === "defensiveShape" ? "organizacion defensiva" : "trabajo equilibrado"}.`,
      recommendedFocus,
      recommendedLoad,
  ...
  for (const player of game.players ?? []) {
    ...
    if (player.injured || (player.medical?.phase && player.medical.phase !== "available")) {
      ...
      pushEvent(events, {
        ...
        summary:`${player.medical?.type ? `Tiene una ${player.medical.type.toLowerCase()}` : "Esta lesionado"}. Prevemos unos ${formatMedicalDuration(remaining)} de baja.`,
        subjectId:player.id,
        subjectName:player.name,
    ...
    if (!player.injured && risk >= 76) {
      pushEvent(events, {
        ...
        summary:`Tiene un riesgo ${ risk >= 90 ? "muy alto" : risk >= 83 ? "alto" : "elevado"} de lesion. Conviene revisar su carga antes del proximo partido.`,
        subjectId:player.id,
        subjectName:player.name,
        action:{ screen:"medical", playerId:player.id },
        actionLabel:"Ver informe medico",
  ...
  for (const prospect of game.youth?.players ?? []) {
    if ((prospect.potential ?? 0) >= 78) {
      pushEvent(events, {
        ...
        summary:`${prospect.pos} de ${prospect.age} años con un potencial ${ (prospect.potential ?? 0) >= 85 ? "enorme" : (prospect.potential ?? 0) >= 80 ? "muy alto" : "alto"}. Merece seguimiento del primer equipo.`,
