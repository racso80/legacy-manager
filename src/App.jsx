...
const REAL_SQUADS = {
  ...
  barcelona:[
    _p("bar-1","Wojciech SzczÃ„â„¢sny","POR","POR",84,35,"PL",55,20,62,38,16,84),
  ...
  celta:[
    ...
    _p("cel-2","IonuÃˆâ€º Radu","POR","POR",73,28,"RO",56,16,54,36,14,73),
  ...
  valladolid:[ // Real Oviedo
    ...
    _p("ovi-2","HoraÃˆâ€ºiu Moldovan","POR","POR",75,28,"RO",56,18,58,36,14,75),
...
function generateSegmentEvents(segment, players, userStr, oppStr, score, tactics = DEFAULT_TACTICS, userIsHome = true, oppSquad = [], medicalContext = {}) {
  ...
  if(isLate && userGoalsNow !== oppGoalsNow && Math.random()<intervalProbability(.16,intervalMinutes)){
    ...
    if(actor)addNarrativeEvent(winningUser?"user":"opp","TIME_WASTING",`${actor.name} intenta enfriar el partido y araÃ±ar unos segundos al reloj.`,actor.id);
  }
...
function CloudSyncIndicator({ session, syncState, conflict, onClick }) {
  ...
  const statusByState = {
    ...
    error: { label:"Error de sincronizaciÃ³n", color:"#ef4444" },
...
const COUNTRIES = [
  ...
  { id: "en", name: "Inglaterra", flag: "ðŸ´", available: false },
...
function ConversationScreen({ conversation, onRespond, onBack }) {
  if (!conversation) {
    return <div style={{ flex:1, padding:18, color:"#9aa0b4" }}>No hay conversaciÃ³n disponible.</div>;
  }
  const priorityMeta = {
    urgent:{ label:"Urgente", color:"#ef4444", icon:"ðŸ”´" },
    important:{ label:"Importante", color:"#f59e0b", icon:"ðŸŸ " },
    info:{ label:"Informativa", color:"#22c55e", icon:"ðŸŸ¢" },
  }[conversation.priority] ?? { label:"Importante", color:"#f59e0b", icon:"ðŸŸ " };
  const effectSummary = option => {
    ...
    if (effects.fanSupport) chunks.push(`${effects.fanSupport>0?"+":""}${effects.fanSupport} aficiÃ³n`);
    if (option.memory) chunks.push("se recordarÃ¡");
    if (option.navigateTo) chunks.push("abre decisiÃ³n");
      ...
      <div style={{ background:"linear-gradient(145deg,rgba(201,168,76,.16),#161a24 48%)", border:"1px solid rgba(201,168,76,.28)", borderRadius:15, padding:16, marginBottom:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ fontSize:11, color:"#8b92a3", marginTop:2 }}>{conversation.role} Â· {conversation.emotionalState}</div>
          </div>
        </div>
        ...
        {(conversation.options ?? []).map((option, index)=>(
          <button key={option.id} onClick={()=>onRespond(conversation.id, option.id)} style={{ display:"flex", gap:10, alignItems:"flex-start", width:"100%", textAlign:"left", background:"#161a24", border:"1px solid rgba(255,255,255,.08)", borderRadius:12, padding:12, cursor:"pointer" }}>
            ...
            <span style={{ flex:1 }}>
              ...
              <small style={{ display:"block", color:"#6b7280", fontSize:9, marginTop:4 }}>Tono: {option.tone}{effectSummary(option)?` Â· ${effectSummary(option)}`:""}</small>
            </span>
          </button>
        ))}
      </div>
      <button onClick={onBack} className="btn-ghost" style={{ width:"100%", marginTop:12, padding:12, borderRadius:10, fontSize:12 }}>Responder mÃ¡s tarde</button>
    </div>
  );
}

const STAFF_PERSONAS = {
  "Director deportivo": { emoji:"ðŸ‘”", color:"#60a5fa", role:"Director deportivo", personality:"piensa en el largo plazo" },
  "Segundo entrenador": { emoji:"ðŸ‘¥", color:"#c9a84c", role:"Segundo entrenador", personality:"directo y prÃ¡ctico" },
  "MÃ©dico": { emoji:"ðŸ‘¨â€âš•ï¸", color:"#22c55e", role:"MÃ©dico", personality:"prudente" },
  "Preparador fÃ­sico": { emoji:"ðŸ‹ï¸", color:"#f59e0b", role:"Preparador fÃ­sico", personality:"protector con la carga" },
  "CapitÃ¡n": { emoji:"â¤ï¸", color:"#ef4444", role:"CapitÃ¡n", personality:"protege al grupo" },
  "Presidente": { emoji:"ðŸ›ï¸", color:"#a78bfa", role:"Presidente", personality:"exigente" },
  "Responsable de prensa": { emoji:"ðŸŽ™ï¸", color:"#f97316", role:"Jefa de prensa", personality:"mide cada palabra" },
  "PsicÃ³logo": { emoji:"ðŸ§ ", color:"#38bdf8", role:"PsicÃ³logo", personality:"lee el vestuario" },
  "Jefe de gabinete": { emoji:"ðŸ—‚ï¸", color:"#94a3b8", role:"Asistente personal del entrenador", personality:"discreto y ordenado" },
};

function emotionMeta(state = "neutral") {
  const key = String(state).toLowerCase();
  if (key.includes("enfad") || key.includes("dolido") || key.includes("tenso")) return { icon:"ðŸ˜ ", label:"Enfadado", color:"#ef4444" };
  if (key.includes("preocup") || key.includes("serio") || key.includes("inquiet")) return { icon:"ðŸ˜Ÿ", label:"Preocupado", color:"#f59e0b" };
  if (key.includes("agrade") || key.includes("positivo") || key.includes("feliz")) return { icon:"ðŸ˜Š", label:"Contento", color:"#22c55e" };
  if (key.includes("motivad")) return { icon:"ðŸ”¥", label:"Motivado", color:"#f97316" };
  if (key.includes("lesion")) return { icon:"ðŸ¤•", label:"Tocado", color:"#ef4444" };
  return { icon:"ðŸ˜", label:"Neutral", color:"#9aa0b4" };
}
...
function PersonAvatar({ person, size = 42 }) {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:Math.round(size*.28), overflow:"hidden", background:`linear-gradient(145deg,${color}33,#0d0f14)`, border:`1px solid ${color}55`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:`0 0 18px ${color}18` }}>
      {person?.portrait ? <img src={person.portrait} alt={person.name} onError={event=>{event.currentTarget.style.display="none";}} style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"top center" }}/> : <span style={{ fontSize:Math.round(size*.46) }}>{person?.emoji ?? "ðŸ‘¤"}</span>}
    </div>
  );
}
    ...
    <div style={{ flex:1, overflowY:"auto", padding:14, background:"radial-gradient(circle at top,rgba(201,168,76,.08),transparent 38%)" }}>
      <div className="bounce-in" style={{ background:"linear-gradient(145deg,#1a1f2e,#11141c)", border:`1px solid ${(actor.color??"#c9a84c")}44`, borderRadius:18, padding:18, marginBottom:13, boxShadow:"0 18px 45px rgba(0,0,0,.32)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:15 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:9, color:"#6b7280", fontWeight:900, letterSpacing:".9px" }}>{scene.location} Â· {scene.time}</div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          <div style={{ background:"rgba(255,255,255,.035)", borderRadius:10, padding:10 }}>
            ...
            <div style={{ fontSize:10, color:"#b45309", lineHeight:1.45, marginTop:4 }}>{scene.consequenceIfIgnored ?? "La situaciÃ³n puede volver mÃ¡s adelante con otro tono."}</div>
          </div>
          <div style={{ background:"rgba(255,255,255,.035)", borderRadius:10, padding:10 }}>
      ...
      {!chosenOption ? (
        <>
          <div style={{ fontSize:11, color:"#c9a84c", fontWeight:900, letterSpacing:".6px", margin:"0 0 9px 2px" }}>Â¿QUÃ‰ LE DICES?</div>
          <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
            {(scene.options ?? []).map((option,index)=>(
              <button key={option.id} onClick={()=>setChosenOption(option)} style={{ display:"flex", gap:10, alignItems:"flex-start", width:"100%", textAlign:"left", background:"#161a24", border:"1px solid rgba(255,255,255,.08)", borderRadius:12, padding:12, cursor:"pointer" }}>
                ...
                <span style={{ flex:1 }}>
                  <strong style={{ display:"block", color:"#e8eaf0", fontSize:13, lineHeight:1.35 }}>{option.label}</strong>
                  <small style={{ display:"block", color:"#8f96a8", fontSize:9, marginTop:4, lineHeight:1.35 }}>Transmite: {option.tone} Â· {option.consequence}</small>
                </span>
              </button>
            ))}
          </div>
        ...
        <div className="fade-in" style={{ background:"linear-gradient(145deg,rgba(201,168,76,.12),rgba(255,255,255,.04))", border:"1px solid rgba(201,168,76,.24)", borderRadius:16, padding:14 }}>
          <div style={{ fontSize:10, color:"#c9a84c", fontWeight:950, letterSpacing:".7px", marginBottom:8 }}>SU REACCIÃ“N</div>
          <div style={{ fontSize:15, color:"#f3f4f6", lineHeight:1.5, fontWeight:700, marginBottom:10 }}>
            {chosenOption.reaction ?? `${actor.name ?? "La persona"} asiente. La conversaciÃ³n no cierra todas las dudas, pero deja una decisiÃ³n sobre la mesa.`}
          </div>
...
function attentionPersona(item) {
  ...
  if (item.category === "medical" || title.includes("lesion") || title.includes("riesgo fÃ­sico") || title.includes("fatiga")) {
    return { ...Object.values(STAFF_PERSONAS).find(person=>person.role?.toLowerCase().includes("dico")), name:"MÃ©dico", emotionalState:"preocupado", line:`${item.summary ?? item.title}`, action:item.actionLabel ?? "Ver informe" };
  }
  if (item.category === "contracts" || title.includes("contrato") || title.includes("renov")) {
    ...
    return { ...STAFF_PERSONAS["Director deportivo"], name:"Director deportivo", emotionalState:"serio", line:`He revisado este asunto contractual. ${item.summary ?? "Creo que deberÃ­amos movernos pronto."}`, action:item.actionLabel ?? "Negociar" };
  }
  ...
  if (item.category === "match" || title.includes("alineaciÃ³n") || title.includes("partido") || title.includes("sancionado")) {
    return { ...STAFF_PERSONAS["Segundo entrenador"], name:"Segundo entrenador", emotionalState:"directo", line:item.summary ?? "MÃ­ster, hay algo del prÃ³ximo partido que debemos resolver.", action:item.actionLabel ?? "Preparar" };
  }
  if (item.category === "fans") {
    return { ...STAFF_PERSONAS.Presidente, name:"Presidente", emotionalState:"exigente", line:item.summary ?? "La aficiÃ³n estÃ¡ hablando y conviene escucharla.", action:item.actionLabel ?? "Responder" };
  }
  if (item.category === "staff" || item.category === "training") {
    return { ...STAFF_PERSONAS["Preparador fÃ­sico"], name:"Preparador fÃ­sico", emotionalState:"preocupado", line:item.summary ?? "Tengo una recomendaciÃ³n para el trabajo del equipo.", action:item.actionLabel ?? "Revisar" };
  }
  if (item.category === "board" || item.category === "career") {
    return { ...STAFF_PERSONAS.Presidente, name:"Presidente", emotionalState:"serio", line:item.summary ?? "Necesito comentar contigo una cuestiÃ³n importante.", action:item.actionLabel ?? "Entrar" };
  }
  if (item.category === "youth") {
    return { emoji:"ðŸŒ±", color:"#84cc16", role:"Responsable de cantera", personality:"protege el futuro", name:"Responsable de cantera", emotionalState:"ilusionado", line:item.summary ?? "Hay un chico que merece tu atenciÃ³n.", action:item.actionLabel ?? "Ver cantera" };
  }
...
function clubLifePersona(issue) {
  const actorMap = {
    ...
    doctor: "MÃ©dico",
    fitnessCoach: "Preparador fÃ­sico",
  ...
  const base = STAFF_PERSONAS[actorName] ?? { emoji:"ðŸ‘¤", color:"#c9a84c", role:actorName, personality:"necesita una decisiÃ³n" };
  return {
    ...base,
    name: issue.person?.name ?? actorName,
    role: issue.person?.name ? `${base.name ?? actorName} Â· ${base.role ?? issue.origin}` : base.role,
...
function conversationPersona(conversation) {
  if (conversation.actorType === "player") return { name:conversation.actorName, role:conversation.role, portrait:conversation.portrait, emoji:"ðŸ‘¤", color:"#c9a84c", emotionalState:conversation.emotionalState, line:conversation.opening, action:"Hablar" };
  const base = STAFF_PERSONAS[conversation.actorName] ?? { emoji:"ðŸ‘¤", color:"#c9a84c", role:conversation.role, personality:"profesional" };
...
function Dashboard({ game, onPlay, setScreen, lineup, attentionItems = [], conversations = [], clubLifeIssues = [], directorItems = [], onOpenAttention, onOpenConversation, onOpenClubLifeIssue, onOpenScene }) {
  ...
  const nextOpponentForm = nextOpponentLast.map(f=>{const h=f.homeTeamId===nextOpponent?.id;const gf=h?f.homeGoals:f.awayGoals;const ga=h?f.awayGoals:f.homeGoals;return gf>ga?"V":gf===ga?"E":"D";}).join(" Â· ") || "Sin racha reciente";
  ...
  const agendaItems = [
    nextFixture && { icon:"âš½", title:`Partido de Liga Â· Jornada ${nextFixture.matchday}`, detail:`${nextFixture.homeTeamId===game.teamId?"Recibes a":"Visitas a"} ${nextOpponent?.name ?? "rival por confirmar"}`, action:"match" },
    { icon:"ðŸ‹ï¸", title:"Entrenamiento de la plantilla", detail:`Carga ${game.trainingPlan?.load ?? "media"} Â· revisar si hay fatiga acumulada`, action:"training" },
    (game.matchday<=8||game.matchday>=31) && { icon:"ðŸ’°", title:"Mercado abierto", detail:game.matchday<=8?`Quedan ${Math.max(0,9-game.matchday)} jornadas para el cierre inicial`:`Quedan ${Math.max(0,39-game.matchday)} jornadas para el cierre final`, action:"transfers" },
    urgentAttention.find(item=>item.category==="contracts") && { icon:"ðŸ“„", title:"Contratos pendientes", detail:"Hay decisiones contractuales que requieren revisiÃ³n", action:"contracts" },
    (game.transferMarket?.offers??[]).some(offer=>["clubCounter","playerCounter","ready","clubAccepted"].includes(offer.status)) && { icon:"ðŸ“¬", title:"Negociaciones activas", detail:"Hay respuestas de mercado esperando decisiÃ³n", action:"transfers" },
  ].filter(Boolean).slice(0,4);
  const consequenceItems = [
    game.lastTrainingReport?.improved?.length && { icon:"ðŸ“ˆ", text:`${game.lastTrainingReport.improved[0].name} mejora tras el trabajo semanal.` },
    lockerSummary.atmosphere==="tenso" && { icon:"âš ï¸", text:"El vestuario muestra seÃ±ales de tensiÃ³n y conviene intervenir." },
    lockerSummary.atmosphere==="positivo" && { icon:"ðŸ¤", text:"El vestuario mantiene un clima positivo alrededor del entrenador." },
    fanSupport<45 && { icon:"â¤ï¸", text:"La aficiÃ³n estÃ¡ perdiendo confianza y necesita una reacciÃ³n." },
    highLoadPlayers[0] && { icon:"ðŸ©º", text:`El cuerpo mÃ©dico recomienda descanso para ${highLoadPlayers[0].name}.` },
    latestNews[0] && { icon:"ðŸ“°", text:latestNews[0].summary || latestNews[0].title },
  ].filter(Boolean).slice(0,4);
  const kpiCards = [
    { label:"Vestuario", value:lockerSummary.atmosphere==="tenso"?"Tenso":lockerSummary.atmosphere==="positivo"?"Positivo":"Estable", trend:lockerSummary.unhappy.length?`${lockerSummary.unhappy.length} jugador${lockerSummary.unhappy.length===1?"":"es"} incÃ³modo${lockerSummary.unhappy.length===1?"":"s"}`:"Grupo unido", color:lockerSummary.atmosphere==="tenso"?"#ef4444":lockerSummary.atmosphere==="positivo"?"#22c55e":"#c9a84c", action:"lockerRoom" },
    { label:"AficiÃ³n", value:`${fanSupport}%`, trend:fanSupport>=70?"Ilusionada":fanSupport>=50?"Exigente":"Preocupada", color:fanSupport>=70?"#22c55e":fanSupport>=50?"#f59e0b":"#ef4444", action:"fans" },
    { label:"EconomÃ­a", value:fmtBudget(budgetLeft), trend:budgetLeft>0?"Margen para operar":"Sin margen de fichajes", color:budgetLeft>0?"#22c55e":"#ef4444", action:"finances" },
    { label:"Carga fÃ­sica", value:highLoadPlayers.length?`${highLoadPlayers.length} alertas`:"Controlada", trend:avgFatigue>55?"Fatiga media elevada":"Plantilla recuperando bien", color:highLoadPlayers.length?"#f97316":"#22c55e", action:"medical" },
  ];
  const objectiveItems = [
    { label:"Liga", value:`${pos}Âº Â· ${standing?.points??0} pts`, color:pos<=6?"#22c55e":pos>=17?"#ef4444":"#c9a84c" },
    ...
    <div style={{ flex:1, overflowY:"auto", padding:14 }}>
      ...
      <div style={{ background:waitingPeople.length?"linear-gradient(145deg,rgba(201,168,76,.15),#161a24 50%)":"linear-gradient(145deg,rgba(34,197,94,.11),#161a24 50%)", border:`1px solid ${waitingPeople.length?"rgba(201,168,76,.30)":"rgba(34,197,94,.22)"}`, borderRadius:14, padding:15, marginBottom:12 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, marginBottom:10 }}>
            <div>
              <div style={{ fontSize:10, color:"#6b7280", fontWeight:900, letterSpacing:".8px" }}>REQUIERE TU ATENCIÃ“N</div>
              ...
              <div style={{ fontSize:10, color:"#8b92a3", marginTop:3 }}>{waitingPeople.length?"El club ha seguido trabajando. Ahora necesita una decisiÃ³n.":"No hay asuntos urgentes ahora mismo. El club sigue trabajando."}</div>
            </div>
            ...
            {waitingPeople.map(item=>{const person=item.person;const mood=emotionMeta(person.emotionalState);return (
              <button key={`${item.kind}-${item.id}`} onClick={item.onClick} style={{ display:"flex", alignItems:"center", gap:10, width:"100%", textAlign:"left", background:item.priority==="urgent"||item.priority==="critical"?"rgba(239,68,68,.08)":"rgba(13,15,20,.72)", border:`1px solid ${item.priority==="urgent"||item.priority==="critical"?"rgba(239,68,68,.24)":"rgba(255,255,255,.07)"}`, borderRadius:12, padding:10, cursor:"pointer" }}>
                ...
                <span style={{ flex:1, minWidth:0 }}>
                  ...
                  <span style={{ display:"block", color:"#6b7280", fontSize:9, marginTop:4, lineHeight:1.35 }}>{person.role} Â· {mood.label} Â· {person.personality ?? "necesita una decisiÃ³n"}</span>
                ...
                <span style={{ color:"#c9a84c", fontSize:10, fontWeight:900, whiteSpace:"nowrap" }}>{person.action} â†’</span>
              </button>
            )})}
          </div>}
      </div>
      ...
      {agendaItems.length>0 && (
        <div style={{ background:"#161a24", border:"1px solid rgba(255,255,255,.07)", borderRadius:12, padding:14, marginBottom:12 }}>
          <div style={{ fontSize:11, color:"#c9a84c", fontWeight:900, letterSpacing:".6px", marginBottom:10 }}>ðŸ“… AGENDA DEL DÃA</div>
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
      ...
      {consequenceItems.length>0 && (
        <div style={{ background:"linear-gradient(145deg,rgba(96,165,250,.08),#161a24 45%)", border:"1px solid rgba(96,165,250,.17)", borderRadius:12, padding:14, marginBottom:12 }}>
          <div style={{ fontSize:11, color:"#60a5fa", fontWeight:900, letterSpacing:".6px", marginBottom:10 }}>ðŸ§­ ÃšLTIMAS CONSECUENCIAS</div>
      ...
      <div style={{ background:"#161a24", border:"1px solid rgba(255,255,255,.07)", borderRadius:12, padding:14, marginBottom:12 }}>
        <div style={{ fontSize:11, color:"#c9a84c", fontWeight:900, letterSpacing:".6px", marginBottom:10 }}>ðŸ“Š ESTADO DEL CLUB</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
        ...
        return (
          <div style={{ background:"#1a1f2e", border:"1px solid rgba(255,255,255,.08)", borderRadius:10, padding:14, marginBottom:12 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:7, marginTop:11 }}>
              {[["PosiciÃ³n rival",nextOpponentStanding?`${nextOpponentStanding}Âº`:"â€”"],["Forma rival",nextOpponentForm],["Jugador peligroso",nextOpponentKeyPlayer?.name??"Sin referencia"],["Importancia",pos<=6||nextOpponentStanding<=6?"Partido de prestigio":pos>=16?"Necesitas puntuar":"Jornada clave"]].map(([label,value])=>(
      ...
      <div style={{ background:"#161a24", border:"1px solid rgba(201,168,76,.18)", borderRadius:12, padding:14, marginBottom:12 }}>
        <div style={{ fontSize:11, color:"#c9a84c", fontWeight:900, letterSpacing:".6px", marginBottom:10 }}>ðŸŽ¯ OBJETIVOS</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
...
function MatchScreen({ game, saveId, tactics: baseTactics, setTactics: setBaseTactics, lineup: baseLineup, setLineup: setBaseLineup, subs: baseSubs, setSubs: setBaseSubs, formation:baseFormation="4-3-3", onMatchEnd, onAbandonMatch }) {
  ...
  const eventLabels  = { GOAL:"GOL",PENALTY:"PENALTI",BIG_CHANCE:"OCASIÃ“N",YELLOW:"AMARILLA",RED:"ROJA",SAVE:"PARADA",DEFENSIVE_ACTION:"DEFENSA",INJURY:"LESIÃ“N",SUBSTITUTION:"CAMBIO",ADDED_TIME:"DESCUENTO",HALFTIME:"DESCANSO",LIVE_DECISION:"DECISION",CORNER:"CÃ“RNER",DANGEROUS_CROSS:"CENTRO",BLOCKED_SHOT:"BLOQUEO",OFFSIDE:"FUERA J.",DANGEROUS_FOUL:"FALTA",TIME_WASTING:"TIEMPO",PLAYER_KNOCK:"TOCADO",REF_WARNING:"AVISO",PROTEST:"PROTESTA",BENCH_WARMUP:"BANQUILLO",TACTICAL_SHIFT:"TACTICA",CROWD_PRESSURE:"GRADA" };
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ background: "#1a1f2e", padding: "14px 16px", textAlign: "center", borderBottom: "1px solid rgba(255,255,255,.06)", flexShrink: 0 }}>
        ...
        <div style={{ marginTop:6, fontSize:9, color:"#22c55e", fontWeight:700 }}>
          Partido guardado{matchAutosaveAt ? ` Â· ${new Date(matchAutosaveAt).toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"})}` : ""}
        </div>
      </div>
                ...
                <div style={{ marginTop:9, display:"flex", flexDirection:"column", gap:6 }}>
                  {visibleLiveSignals.map(signal => (
                    <button key={signal.key} onClick={() => { setLiveDecision(signal); setPlaying(false); setTab(signal.targetTab ?? "eventos"); }} style={{ textAlign:"left", background: signal.severity==="urgent" ? "rgba(239,68,68,.12)" : "rgba(96,165,250,.1)", border:`1px solid ${signal.severity==="urgent"?"rgba(239,68,68,.3)":"rgba(96,165,250,.25)"}`, borderRadius:8, padding:"8px 9px", cursor:"pointer" }}>
                      <div style={{ fontSize:10, color:signal.severity==="urgent"?"#ef4444":"#60a5fa", fontWeight:900 }}>{signal.source==="doctor"?"Medico":"Segundo entrenador"} Â· {signal.action}</div>
...
function applyAiPhysicalAfterMatch(teamId, formation = "4-3-3") {
  ...
  const activeMatchAttention = activeMatchForGame ? [{
    ...
    summary:"Hay un partido iniciado que todavÃ­a no ha terminado.",
    status:"new",
    action:{ screen:"match" },
  ...
  const directorCandidates = game ? [
    ...
    ...activeConversations.map(conversation=>({
      ...
      topicKey:conversation.actorType==="player"?`player:${conversation.actorId}`:conversation.context==="Plantilla"?"locker-room":conversation.context==="Mercado"?"market-decision":conversation.context==="Cuerpo tÃ©cnico"?"match-preparation":conversation.context,
    ...
    ...activeMatchAttention.map(item=>({
      ...
      status:item.status,
      requiresDecision:true,
      consequenceIfIgnored:"El partido no podrÃ¡ cerrarse correctamente hasta retomarlo o abandonarlo.",
      attention:item,
    })),
  ...
  const headerTitle = {
    ...
    playerProfile: selectedPlayer?.name ?? "Perfil de jugador", conversation: selectedConversation?.actorName ?? "ConversaciÃ³n", scene: selectedScene?.actor?.name ?? "Escena",
  };
        ...
        <div style={{ position:"fixed", inset:0, zIndex:80, background:"rgba(6,8,12,.82)", display:"flex", alignItems:"center", justifyContent:"center", padding:18 }}>
          <div style={{ width:"100%", maxWidth:420, background:"#161a24", border:"1px solid rgba(201,168,76,.28)", borderRadius:16, padding:18, boxShadow:"0 18px 45px rgba(0,0,0,.45)" }}>
            ...
            <div style={{ fontSize:12, color:"#9aa0b4", lineHeight:1.5, marginBottom:14 }}>
              Puedes continuar exactamente donde estaba o abandonar el partido pendiente. No se reiniciarÃ¡ automÃ¡ticamente.
            </div>
            <div style={{ background:"#0d0f14", border:"1px solid rgba(255,255,255,.06)", borderRadius:10, padding:10, marginBottom:14, fontSize:11, color:"#8b92a3" }}>
              Jornada {recoverableMatch.matchday ?? "â€”"} Â· Temporada {recoverableMatch.season ?? "â€”"}
            </div>
