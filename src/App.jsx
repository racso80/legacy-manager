...
import { buildPlayerLookup, generateBoardNews, generateContractNews, generateDevelopmentNews, generateMarketUpdateNews, generateMatchdayNews, generateMedicalNews, generateScoutingNews, generateTransferNews, generateYouthNews, getDashboardNews, mergeNews } from "./news/newsEngine.js";
...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  @media (min-width: 1024px) {
    ...
    .pc-dash-v2-cal-nav-btn { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.07); border-radius: 3px; color: rgba(255,255,255,0.3); font-size: 9px; width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; cursor: pointer; }
    .pc-dash-v2-cal-weeks { display: flex; flex-direction: column; gap: 4px; flex: 1; min-height: 0; }
    .pc-dash-v2-cal-week { display: grid; grid-template-columns: repeat(7, 1fr); gap: 3px; flex: 1; min-height: 0; }
    .pc-dash-v2-cal-day { background: var(--dv2-bg-raised); border: 1px solid rgba(255,255,255,0.04); border-radius: 5px; padding: 3px 2px; display: flex; flex-direction: column; align-items: center; min-width: 0; min-height: 92px; }
    ...
    .pc-dash-v2-ce-icon { font-size: 32px; line-height: 1; }
    .pc-dash-v2-cal-crests { display: flex; justify-content: center; align-items: center; gap: 3px; margin-top: 1px; flex-shrink: 0; }
    ...
    /* ─── PC Centro de Fichajes (Mercado + Scouting unificados) ──
       Tokens reutilizados de los que ya usan Dashboard/Squad/Standings/el shell,
       no una paleta propia: --tc-gold es var(--club-accent) (mismo color que topbar/
       sidebar/OVR de plantilla), fondo de panel #111726 (Standings/Dashboard-v2),
       bordes rgba(255,255,255,.07), radios 6-8px, pills activas con relleno sólido
       igual que .pc-squad-filter-pill[data-active]. */
    .pc-tc-root {
      --tc-gold: var(--club-accent, #c9a84c);
      --tc-text-on-accent: var(--club-text-on-accent, #1a1200);
      --tc-gold-soft: color-mix(in srgb, var(--club-accent, #c9a84c) 14%, transparent);
      --tc-gold-border: color-mix(in srgb, var(--club-accent, #c9a84c) 40%, transparent);
      --tc-panel: #111726;
      --tc-card: #13161e;
      --tc-line: rgba(255,255,255,.07);
      --tc-pitch: #061206;
      --tc-text-dim: #8b92a3;
      --tc-green: #22c55e;
      --tc-red: #ef4444;
    ...
    .pc-tc-header h1 { font-size: 19px; font-weight: 700; color: #e8eaf0; }
    .pc-tc-budget-box { background: var(--tc-panel); border: 1px solid var(--tc-line); border-radius: 8px; padding: 8px 16px; text-align: right; }
    ...
    .pc-tc-main-tab { padding: 8px 18px; border-radius: 6px; font-size: 12px; font-weight: 700; cursor: pointer; color: #9aa0b4; background: #1e2330; transition: background .15s, color .15s; }
    .pc-tc-main-tab.active { background: var(--tc-gold); color: var(--tc-text-on-accent); }

    .pc-tc-sub-tabs { display: flex; gap: 6px; margin-bottom: 16px; }
    .pc-tc-sub-tab { padding: 7px 13px; border-radius: 6px; font-size: 11.5px; font-weight: 600; cursor: pointer; color: #9aa0b4; background: #1e2330; border: none; transition: background .15s, color .15s; }
    .pc-tc-sub-tab.active { background: var(--tc-gold); color: var(--tc-text-on-accent); }

    .pc-tc-hub-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; }
    .pc-tc-hub-card { background: var(--tc-panel); border: 1px solid var(--tc-line); border-radius: 8px; padding: 16px 18px; display: flex; flex-direction: column; gap: 10px; }
    .pc-tc-hub-card-head { display: flex; justify-content: space-between; align-items: center; }
    .pc-tc-hub-title { font-size: 11px; font-weight: 900; letter-spacing: .5px; color: var(--tc-gold); }
    ...
    .pc-tc-hub-big { font-size: 24px; font-weight: 800; color: #e8eaf0; }
    ...
    .pc-tc-hub-row .r-name { font-weight: 600; color: #e8eaf0; }
    ...
    .pc-tc-p-name { font-size: 13px; font-weight: 700; color: #e8eaf0; }
    .pc-tc-p-sub { font-size: 10px; color: var(--tc-text-dim); margin-top: 2px; }
    .pc-tc-deal-badge { font-size: 9px; font-weight: 700; padding: 3px 8px; border-radius: 999px; width: fit-content; }
    .pc-tc-deal-badge.transfer { background: var(--tc-gold-soft); color: var(--tc-gold); }
    .pc-tc-deal-badge.loan { background: rgba(96,165,250,.14); color: var(--tc-blue); }
    .pc-tc-deal-badge.free { background: rgba(34,197,94,.14); color: var(--tc-green); }
    ...
    .pc-tc-empty { grid-column: 1 / -1; background: var(--tc-panel); border-radius: 8px; padding: 24px; text-align: center; font-size: 11.5px; color: var(--tc-text-dim); }
    ...
    .pc-tc-offer-list-item { background: var(--tc-panel); border: 1px solid var(--tc-line); border-radius: 8px; padding: 11px 13px; cursor: pointer; }
    .pc-tc-offer-list-item:hover { border-color: var(--tc-gold-border); }
    .pc-tc-offer-list-item.selected { border-color: var(--tc-gold); background: var(--tc-gold-soft); }
    .pc-tc-offer-list-item .ol-name { font-size: 12px; font-weight: 700; color: #e8eaf0; }
    .pc-tc-offer-list-item .ol-club { font-size: 9.5px; color: var(--tc-text-dim); margin-top: 2px; }
    .pc-tc-offer-list-item .ol-status { font-size: 9px; font-weight: 700; margin-top: 6px; display: inline-block; padding: 2px 7px; border-radius: 999px; }
    .pc-tc-offer-list-item .ol-status.pending { background: var(--tc-gold-soft); color: var(--tc-gold); }
    .pc-tc-offer-list-item .ol-status.good { background: rgba(34,197,94,.14); color: var(--tc-green); }
    .pc-tc-offer-list-item .ol-status.bad { background: rgba(239,68,68,.14); color: var(--tc-red); }

    .pc-tc-detail-panel { background: var(--tc-panel); border: 1px solid var(--tc-line); border-radius: 8px; padding: 22px; max-width: 560px; }
    ...
    .pc-tc-offer-player { font-size: 13.5px; font-weight: 700; color: #e8eaf0; }
    ...
    .pc-tc-status-line { display: flex; align-items: center; gap: 8px; font-size: 12px; margin-bottom: 13px; color: #e8eaf0; }
    ...
    .pc-tc-advisor-box { background: var(--tc-card); border: 1px solid var(--tc-line); border-radius: 8px; padding: 13px 15px; margin-bottom: 13px; display: flex; gap: 11px; }
    ...
    .pc-tc-advisor-note { font-size: 11.5px; color: #e8eaf0; line-height: 1.5; margin-bottom: 5px; }
    ...
    .pc-tc-contract-form input, .pc-tc-contract-form select { background: #0d0f14; border: 1px solid rgba(255,255,255,.1); color: #e8eaf0; border-radius: 6px; padding: 8px 9px; font-size: 12px; }
    ...
    .pc-tc-club-offer-form input { background: #0d0f14; border: 1px solid rgba(255,255,255,.1); color: #e8eaf0; border-radius: 6px; padding: 9px 10px; font-size: 12px; }
    ...
    .pc-tc-mission-form select, .pc-tc-mission-form input { width: 100%; background: var(--tc-card); border: 1px solid var(--tc-line); color: #e8eaf0; padding: 8px 9px; border-radius: 6px; font-size: 11.5px; }
    .pc-tc-mission-submit { margin-top: 4px; padding: 9px; border-radius: 6px; font-size: 12px; }
    ...
    .pc-tc-report-card { background: var(--tc-panel); border: 1px solid var(--tc-line); border-radius: 8px; padding: 15px 17px; }
    .pc-tc-report-top { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; }
    .pc-tc-report-name { font-size: 13.5px; font-weight: 700; color: #e8eaf0; }
    .pc-tc-report-meta { font-size: 10.5px; color: var(--tc-text-dim); margin-top: 2px; }
    .pc-tc-talent-badge { font-size: 9.5px; font-weight: 700; padding: 3px 9px; border-radius: 999px; flex-shrink: 0; }
    ...
    .pc-tc-range-block .val { font-size: 14px; font-weight: 700; color: #e8eaf0; display: block; }
    ...
    .pc-tc-confidence-fill { height: 100%; background: linear-gradient(90deg, var(--tc-gold-border), var(--tc-gold)); }
...
function LineupScreen({ game, players, lineup, setLineup, formation, setFormation, subs, setSubs, savedLineups, onSaveLineups, onOpenPlayer, isPC, onPlay }) {
  ...
  // ── 4 + 5. Gestión de un titular: quitar del once, o intercambiar con otro titular / banquillo / no convocado ──
  ...
  const renderCandidateRow = (p, onClick) => {
    ...
    return (
      <div key={p.id} onClick={onClick}
        style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 9px", marginBottom:5, background:"#161a24", border:"1px solid rgba(255,255,255,.07)", borderRadius:7, cursor:"pointer", flexShrink:0 }}>
    ...
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <div style={{ borderBottom:"1px solid rgba(255,255,255,.06)", flexShrink:0 }}>
        ...
        <div style={{ display:"flex", gap:6, padding:"0 12px 8px" }}>
          ...
          <button onClick={() => setConfirmClearLineup(true)}
            style={{ background:"transparent", border:"1px solid rgba(239,68,68,.25)", color:"rgba(239,68,68,.85)", padding:"7px 10px", borderRadius:6, fontSize:11, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap" }}>
            🗑 Borrar alineación
          </button>
        </div>

        {confirmClearLineup && (
          <div style={{ margin:"0 12px 8px", display:"flex", alignItems:"center", gap:8, background:"rgba(239,68,68,.08)", border:"1px solid rgba(239,68,68,.25)", borderRadius:8, padding:"8px 10px" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          {/* ── 5. Panel de gestión del titular (al pulsar titular): quitar o intercambiar ── */}
          {subTarget && (
            <div style={{ background:"#1a1f2e", borderBottom:"1px solid rgba(201,168,76,.25)", padding:"10px 12px", flexShrink:0, display:"flex", flexDirection:"column", maxHeight:"48vh" }}>
              ...
              <div style={{ overflowY:"auto", paddingRight:2 }}>
                <button onClick={removeStarter}
                  style={{ display:"flex", alignItems:"center", justifyContent:"center", width:"100%", gap:6, padding:"8px 9px", marginBottom:10, background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.3)", borderRadius:7, color:"#ef4444", fontSize:12, fontWeight:700, cursor:"pointer" }}>
                  ❌ Quitar del once
                </button>

                <div style={{ fontSize:9, fontWeight:800, color:"#22c55e", letterSpacing:".4px", marginBottom:5 }}>TITULARES</div>
                {getStarterCandidates().length === 0 && (
                  <div style={{ fontSize:10, color:"#4b5563", padding:"4px 0 8px" }}>No hay otros titulares</div>
                )}
                {getStarterCandidates().map(p => renderCandidateRow(p, () => swapStarters(lineup.indexOf(p.id))))}

                <div style={{ fontSize:9, fontWeight:800, color:"#fbbf24", letterSpacing:".4px", margin:"10px 0 5px" }}>BANQUILLO</div>
                {getBenchCandidates().length === 0 && (
                  <div style={{ fontSize:10, color:"#4b5563", padding:"4px 0 8px" }}>No hay suplentes disponibles</div>
                )}
                {getBenchCandidates().map(p => renderCandidateRow(p, () => swapWithBench(p)))}

                <div style={{ fontSize:9, fontWeight:800, color:"#9aa0b4", letterSpacing:".4px", margin:"10px 0 5px" }}>NO CONVOCADOS</div>
                {getNotCalledCandidates().length === 0 && (
                  <div style={{ fontSize:10, color:"#4b5563", padding:"4px 0" }}>No hay jugadores no convocados</div>
                )}
...
function applyAiPhysicalAfterMatch(teamId, formation = "4-3-3") {
  ...
  const handleMatchEnd = (fixtureId, homeGoals, awayGoals, events, livePlayer, participation) => {
    ...
    setGame(prev => {
      ...
      const lockerNewsItems=lockerSummary.atmosphere==="tenso"?[{title:"El vestuario muestra señales de tensión",summary:`Hay ${lockerSummary.unhappy.length} jugador${lockerSummary.unhappy.length===1?"":"es"} con preocupación interna. Conviene revisar moral, minutos o contratos.`,importance:"high",fingerprint:`locker:tension:${prev.season}:${matchday}`}]:lockerSummary.atmosphere==="positivo"&&won?[{title:"El vestuario respalda al entrenador",summary:"El ambiente interno es positivo y los líderes mantienen unido al grupo.",importance:"medium",fingerprint:`locker:positive:${prev.season}:${matchday}`}]:[];
      const lockerNews=generateBoardNews({items:lockerNewsItems,season:prev.season??"2025",matchday,userTeamId:prev.teamId});
      ...
      if((newGame.transferMarket?.incomingOffers?.length??0)>incomingBefore){const offer=newGame.transferMarket.incomingOffers[0];const buyer=TEAMS.find(team=>team.id===offer.toTeamId);const[item]=generateMarketUpdateNews({items:[{title:`📬 Oferta del ${buyer?.name} por ${offer.playerName}`,summary:`El club ofrece €${(offer.amount/1000).toFixed(1)}M por ${offer.type==="loan"?"su cesión":"el traspaso"}.`,importance:"high",fingerprint:offer.id}],season:newGame.season,matchday,userTeamId:newGame.teamId});newGame={...newGame,news:mergeNews(newGame.news??[],[item]),transferMarket:{...newGame.transferMarket,notifications:[{id:item.id,title:item.title,status:"unread",matchday},...(newGame.transferMarket.notifications??[])]}};}
  ...
  const handleClauseTrigger=(player,fromTeamId)=>updateTransferMarket(prev=>triggerReleaseClause(prev,{player,fromTeamId}));
